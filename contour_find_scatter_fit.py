...
#for c in range(3,4):
    ...
    #plt.savefig('C:/Users/Brendan/Desktop/contours_Car_Rot_%i_%i.pdf' % ((c*rotations)+1, ((c+1)*rotations)))
    plt.close()       
    ...
    ARs = np.zeros((len(paths),3,500))  # frame, long, int
    ARs_corrected = np.zeros((len(paths),3,500))  # frame, long, int
    ...
    #for i in range(9):
       ...
       for c1 in range(matrx.shape[0]):
           points = zip([r for r in range(matrx.shape[0])],[c1 for q in range(matrx.shape[1])])
    ...
    #for k in range(1):
       ...
       m, b = np.polyfit(frames, longitudes, 1)
       fit_params[k] = [m, b]
        
       ax1.plot(frames, m*frames + b, 'r-')  
       
       for t in range(len(frames)):
           #ARs_corrected[k,1,t] = ARs[k,1,t] - (ARs[k,0,t] - np.min(frames)) * m  # correct to start of band
           ARs_corrected[k,1,t] = ARs[k,1,t] - (ARs[k,0,t] - np.min(ARs[k,0,:])) * m  # correct to start of plot
           AR_total[k+cumulative_bands,1,t] = ARs[k,1,t] - (ARs[k,0,t] - np.min(ARs[k,0,:])) * m  # correct to start of plot
    ...
    #"""
    #plt.savefig('C:/Users/Brendan/Desktop/AR_Bands_Compare_Car_Rot_%i_%i.pdf' % ((c*rotations)+1, ((c+1)*rotations)))
    plt.close()    
    ...
    #plt.savefig('C:/Users/Brendan/Desktop/AR_Histogram_Compare_Car_Rot_%i_%i.pdf' % ((c*rotations)+1, ((c+1)*rotations)))
    plt.close()
